...
int main(int argc, char** argv) {
    ...
    std::fprintf(stderr, "cublas: n=%d d=%d epochs=%d -> mse=%.6f, max_weight_err=%.6f, %.4fs compute (+%.4fs H2D transfer)\n",
                 n, d, epochs, mse, max_w_err, secs, xfer_s);
    std::printf("method=cublas,n=%d,d=%d,epochs=%d,mse=%.8f,max_weight_err=%.8f,time=%.6f,transfer_time=%.6f\n",
                n, d, epochs, mse, max_w_err, secs, xfer_s);
