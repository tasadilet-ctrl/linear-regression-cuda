...
int main(int argc, char** argv) {
    ...
    std::fprintf(stderr, "cuda_naive: n=%d d=%d epochs=%d -> mse=%.6f, max_weight_err=%.6f, %.4fs\n",
                 n, d, epochs, mse, max_w_err, secs);
    std::printf("method=cuda_naive,n=%d,d=%d,epochs=%d,mse=%.8f,max_weight_err=%.8f,time=%.6f\n",
                n, d, epochs, mse, max_w_err, secs);
